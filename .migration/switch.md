...
- `src/renderer/components/ui/switch.tsx` - `radix-ui` Switch -> `@base-ui/react/switch`; the
  thumb is `SwitchPrimitive.Thumb`. Project class string preserved.
- Leftover scan clean.
...
- State attributes move from `data-state=checked` to `data-checked` /
  `data-unchecked`, and disabled styling from `disabled:` to `data-disabled:`.
  The golden classes already match.

## Verify by hand

1. Settings: flip a few switches, including the compilation and MO2 ones.
2. Check the thumb animates and a disabled switch is still dimmed.
