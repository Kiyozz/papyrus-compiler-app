...
const actions = {
  ...
  groupsPage: {
    ...
    saveToLocal: createAction<GroupModel[]>(
      CONSTANTS.APP_GROUPS_SAVE_GROUPS_TO_LOCAL
    ),
    add: createAction<GroupModel>(CONSTANTS.APP_GROUPS_ADD_GROUP),
    edit: createAction<{ group: GroupModel; lastName: string }>(
      CONSTANTS.APP_GROUPS_EDIT_GROUP
    ),
  ...
  settingsPage: {
    ...
    mo2: {
      ...
      detectSources: {
        start: createAction<[string, string]>(
          CONSTANTS.APP_SETTINGS_DETECT_SOURCES_FOLDERS
        ),
        success: createAction<string[]>(
